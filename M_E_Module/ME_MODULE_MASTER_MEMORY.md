...
|-------|----------------------|------------------|--------|
| **Orchestrator** | Inventory uploads, dispatch agents via the dispatch wrapper, hold state via `report_jobs`, halt at gates | **Plain Python** (not an LLM agent) | **BUILT & PROVEN END-TO-END** — full Stage E walk classify→extract→reconcile→Gate 1→E3→Gate 2→synthesise-park proven on real FCDO data in prod |
...
| **Proposal extractor (D2)** | Objectives, activities, original indicators+targets from the winning proposal | **Agent SDK (subprocess)** | **BUILT & proven** — now has **retry-then-degrade resilience** (parity with D3/D4): a timeout no longer hard-fails the stage (see §8b) |
...
| **Tabular/indicator extractor (D4)** | Actuals-vs-targets, beneficiary counts, disaggregation, financials from Excel/CSV | **Agent SDK (subprocess)** | **BUILT & proven** — accepts spreadsheet input only (.xlsx/.csv); an unsupported/unparseable format (e.g. .docx table) now **degrades gracefully** (parity with D2/D3) instead of killing the stage (see §8b). **Reading .docx tables is a deferred capability**, not built. |
...
| **Knowledge-bank reconciler (E1)** | Merge extractions into one picture; **surface conflicts** (don't silently average) | **Messages API** | **BUILT & PROVEN IN PROD** — produced a clean 33-fact bank on the full real FCDO set (`reconciliation_outcome=complete`). Has **degrade pass-through**: if the model output fails to parse but candidates exist, carries them forward as unconfirmed (`confirmed=False`, `coverage="single_source"`) rather than writing an empty KB. **Known intermittent quality item:** the model occasionally emits invalid/oversized JSON (~48k chars vs `max_tokens=16384`) → degrade; netted by pass-through; parked for V2 (see §17, V2 brief). |
| **Gap/compliance agent (E3)** | Compare knowledge bank vs funder template; readiness score + funder-aware questions | **Messages API** | **BUILT, WIRED & PROVEN IN PROD** — runs in the orchestrated gap stage after Gate 1 resume; surfaced FCDO-shaped gaps and halted at Gate 2 on real data |
| **Synthesis agents** | Write each section (one per section); reuse archetypes + humaniser | gpt-5.4 (humaniser reuse) | **NOT STARTED** (Stage F) — next work |
...
```
Gates are **orchestrator state-machine halts enforced server-side** — **not** Agent SDK hooks. The orchestrator halts a job by setting `report_jobs.status = awaiting_human` with the `stage` cursor parked at the gate; the pipeline cannot advance until the human confirmation route stamps the gate (e.g. `gate1_confirmed_at`) and **re-enqueues** the job (`status → queued`) for the worker to re-claim and continue. **Cursor convention (locked):** the gate guarding stage X is encoded as `(awaiting_human, stage=X)` — Gate 1 guards the gap stage `(awaiting_human, gap)`; Gate 2 guards synthesis `(awaiting_human, synthesise)`. **Gate 1 and Gate 2 are both built and proven in prod** — full confirm → re-enqueue → re-claim → advance, with Gate 2's `re_enqueue_gate2_job` mirroring Gate 1's. After Gate 2 the worker parks at the **synthesise boundary** (Stage F not built) as a resumable checkpoint.
...
4. **Extractor resilience must be uniform — and wasn't.** Three separate hard-fail-instead-of-degrade defects surfaced on real prod documents, each invisible to CI: (a) **D2** leaked a bare `asyncio.TimeoutError` and had no retry/degrade — a slow proposal killed the whole extract stage; (b) **D4** raised an uncaught `ValueError: Unsupported spreadsheet format: .docx` on a logframe submitted as a Word table — same stage-kill; (c) **E1** wrote an *empty* knowledge bank on a JSON-parse failure even when good extractor candidates existed. All three are now fixed to **degrade-and-continue** (D2/D4 retry-then-degrade; E1 carries candidates forward as unconfirmed). **Lesson: a "degrade gracefully" contract is only real if EVERY agent implements it — parity must be checked agent-by-agent, not assumed from one sibling.** The real documents (large proposal, .docx logframe, full bundle) triggered every one of these; the test fixtures never did.
...
6. **Alembic revision IDs must fit VARCHAR(32).** Confirmed clean on this build's deploys (max id `0013_ngo_profiles_knowledge_bank` = 32 chars exactly). Verbose IDs overflow `alembic_version.version_num` and boot-loop the worker (the `0015` overflow, since fixed). Pre-deploy check: enumerate all revision IDs, confirm ≤32 and single linear head.
7. **The local scratch-Postgres migration harness is blocked** (missing `pgcrypto` on the dev host) — harmless while deploys are app-only (no new migrations), but **fix before any future migration deploy** so the executable migration guard works when it's actually needed.
8. **`psql` is unavailable on the Windows dev host** ([WinError 2]) — the prod smoke-walk script reports `OVERALL=FAIL` on its DB-read and Gate-2 auto-close steps for this reason alone. Beats are confirmed via authenticated API readback instead. Tooling artefact, not a pipeline failure; worth hardening the harness to not depend on local `psql`.

**Now PROVEN in prod (was carried-forward):** the Gate 1 **resume** path (re-enqueue → re-claim → advance) — its first real prod exercise was the E3 build's walk, and it ran E3 rather than re-parking. Gate 2 resume + synthesise-boundary park likewise proven on real data.
...
### Stage E — The brain (reconciliation + gaps + gates 1–2) ✅ COMPLETE & PROVEN IN PROD
Knowledge-bank reconciler (surfaces conflicts) → Gate 1 (confirm facts, server-enforced) → gap/compliance agent → Gate 2 (fill only gaps, server-enforced). **All built, wired into the orchestrator, and proven end-to-end on real FCDO data in production:** a real report walked create → upload (4 docs) → classify → extract (with D2/D4 degrade resilience exercised) → reconcile (E1, 33-fact bank, `complete`) → **Gate 1** confirm → resume → **E3** gap stage (FCDO-shaped gaps, readiness scored) → **Gate 2** confirm → re-enqueue → parked at the **synthesise boundary**. Two parked quality items carried forward (see §17): E1's intermittent JSON-parse failure (netted by pass-through) and the indicator `.xlsx` extractor degrading on the test runs.

### Stage F — Generation + critic + THE QUALITY GATE ◄ NEXT WORK
Synthesis agents (reuse archetypes/humaniser, gpt-5.4) → fact-safety critic → Gate 3 (review/edit).
**QUALITY GATE (the plan's hinge):** reports must be **funder-grade on hand-confirmed data** — graded vs humaniser rules, no hallucinated specifics, critic catches planted errors. **No UX polish, no orchestrator, no more templates until this passes.** F consumes the knowledge bank + confirmed gap answers from the parked synthesise checkpoint Stage E now produces; precondition (a real, non-empty KB) is met.
...
- **Synthesis model final call:** start on `gpt-5.4` (humaniser reuse); re-evaluate a cheaper class once synthesis is built, before launch — benchmark in Stage F. *(Still open — Stage F is next.)*
...
- **Gate 1 resume verified in prod:** **RESOLVED** — proven on the E3 build walk (resume ran E3, not a re-park). Gate 2 resume likewise proven.
- **D4 unsupported-format resilience:** **RESOLVED** — unparseable/unsupported formats now degrade gracefully (see §8b.4).
- **Privacy tier (OpenDataLoader):** post-launch consideration, not in MVP.

**Parked quality items (V2 — not MVP blockers; see `ME_MODULE_V2_EXTRACTOR_QUALITY_BRIEF.md`):**
- **E1 reconciler intermittent JSON-parse failure:** the reconciler model occasionally emits invalid/oversized JSON (~48k chars vs `max_tokens=16384`) → degrade. **Bimodal** (same FCDO bundle class passes on the gate fixture; failed twice on one prod run, succeeded on the next). **Netted by E1 pass-through** (carries candidates forward as unconfirmed), so non-fatal. Observability now persists `output_tokens` + raw head/tail snippet on failure — graduates to active work the moment the next prod failure is captured, which also settles the **truncation-vs-malformation** question and therefore the fix class (likely the V2 native-Structured-Outputs / Messages-API lever). Not yet exercised in prod (reconcile succeeded on the latest walk).
- **`.docx` indicator-table READING (D4 capability):** distinct from the resilience fix (done). Reading indicator data out of a Word table — so a logframe submitted as `.docx` is *read*, not just degraded-and-flagged. Real NGOs submit logframes this way; demand-driven, build when usage shows it's common. Belongs in D4's parse path, not D1 re-routing.
- **Indicator `.xlsx` extractor degrading on prod runs:** the spreadsheet extractor (D4) degraded (likely timeout) on the last two walks, so real indicator rows didn't reach the KB even though the pipeline continued. **Highest-priority of the parked items** — indicator data is core to an M&E product. Likely the same latency class as the parked D2 timeout work. Watch the full-read-vs-degrade rate against real usage; decide the fix against that rate.

---

## 18. CURRENT STATUS & NEXT ACTION

- **Status (2026-05-31):** **Stage E is COMPLETE and PROVEN END-TO-END IN PRODUCTION.** The full M&E spine ran on a real FCDO report on the live Railway environment: create → upload (4 docs) → classify (D1) → extract (D2/D3/D4) → reconcile (E1) → **Gate 1** confirm → resume → **E3** gap stage → **Gate 2** confirm → re-enqueue → **parked at the synthesise boundary**. Headline: the knowledge bank held **33 real facts** (`reconciliation_outcome=complete`); E3 surfaced FCDO-shaped gaps (readiness 9, 44 template-driven gaps) and halted server-side at `(awaiting_human, synthesise)`. Stages A–E built and deployed (`ME_MODULE_ENABLED=true`, worker live, live commit `d26278c`).
- **What's built & proven:** D1–D4 extractors (with D2/D4 degrade resilience), E1 reconciler (with degrade pass-through + failure observability), E3 gap agent (orchestrator-wired), Gate 1 + Gate 2 (both full: confirm + re-enqueue + resume), the synthesise-boundary park, the full orchestrated walk, the uniform dispatch wrapper, report-lifecycle HTTP routes, the hardened worker.
- **Immediate next action: SPEC AND BUILD STAGE F** — synthesis agents + fact-safety critic + Gate 3, the plan's **quality-gate hinge**. F consumes the knowledge bank + confirmed gap answers from the parked synthesise checkpoint Stage E produces. Precondition (a real, non-empty KB) is now met. Approach: break F into scoped slices the way E was built (hardening/prereqs → forward walk → resume trigger); Cursor builds; prompts specify WHAT not HOW; verify each slice before the next. **The quality gate stands: no UX polish, no export, no more templates until reports are funder-grade on hand-confirmed data and the critic catches planted errors.**
- **Parked quality items (V2, NOT F blockers — see §17 and `ME_MODULE_V2_EXTRACTOR_QUALITY_BRIEF.md`):** E1 intermittent JSON-parse failure (netted by pass-through; instrumented, awaiting next prod failure to settle fix class); `.docx` indicator-table *reading* capability (deferred); indicator `.xlsx` extractor degrading on prod runs (highest-priority quality watch — indicator data is core; likely same latency class as D2 timeout work).
- **Carried-forward housekeeping (pre-launch):** turn `TEST_MODE` off + kill the `/api/auth/test-mode/mint` endpoint; fix the local scratch-Postgres migration harness (`pgcrypto`) before any future migration deploy; harden the smoke-walk script to not depend on local `psql`.
- **After F + quality gate:** export (H) → frontend (I) → billing (J) → launch (K).
...
- `ORCHESTRATOR_SEAM_AUDIT_2026-05-30.md`, `E3_GAP_GATE2_SEAM_AUDIT_2026-05-31.md` — read-only seam audits backing the orchestrator and the (now-complete) E3/Gate 2 build.
- `ME_MODULE_V2_EXTRACTOR_QUALITY_BRIEF.md` — **parked V2 backlog**: extractor/reconciler quality + speed levers (native Structured Outputs, D2–D4 → Messages API migration, output trim/stream, `.docx` indicator reading, `.xlsx` degrade). Research-backed, NOT MVP. Revisit against real usage data.

---

*End of master memory document. To resume: attach this file and say "continue the M&E build from the project memory file." The assistant resumes at §18 — **Stage E is complete and proven in prod; next work is speccing and building Stage F (synthesis + critic + Gate 3, the quality-gate hinge), broken into scoped slices the way E was built.**
