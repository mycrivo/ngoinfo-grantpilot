...
**Depends on:** DB_FIELD_CONTRACT_FUNDING_OPPORTUNITY.md
**System of Record:** Railway Postgres (GrantPilot DB)
**Primary Driver:** funding_opportunity.requirements_json
**Version:** 1.0.0
...
- `ngo_profile_json`
- `funding_opportunity` flat fields
- `requirements_json` (authoritative contract)
- `user_inputs` (explicit user preferences and uploads)

**Never infer missing requirements.**
Generate only for `requirements_json.variants[*].submission_items`.

If requirements are missing/invalid/empty for `status=READY`, return `DEGRADED_MISSING_REQUIREMENTS` (no speculative output).
...
## 3. STANDARD INPUT OBJECTS (CONTRACT FOR ALL CALLS)

### 3.1 `funding_opportunity` (from DB)

Flat fields per DB contract.
Must include `requirements_json`.

### 3.2 `ngo_profile_json` (from DB/user)

GrantPilot profile object (current stored state).

### 3.3 `user_inputs` (runtime)
...
1. **Select Variant (if not provided):**
   - If user_inputs.selected_variant_id is provided and exists → use it
   - Else apply deterministic selection:
     a. Prefer variant whose eligibility_rules.applicant_type matches ngo_profile.organization_type
     b. Prefer variant with matching geography (ngo_profile.country in variant.eligibility_rules.eligible_countries)
     c. If still ambiguous → select first variant and flag warning: "VARIANT_SELECTION_AMBIGUOUS"

2. **Map Submission Items:**
...
3. **Summarize Required Documents:**
   From variant.required_documents, list status (PROVIDED/MISSING/UNKNOWN) based on user_inputs.uploaded_documents_index
...
You generate content only for the given submission_item from requirements_json.
...
WHO (1 sentence): Organization name + core mission (cite ngo_profile.mission_statement)
WHAT (2-3 sentences): Problem this grant will address (cite local context if in ngo_profile)
...
At least ONE data point from ngo_profile.past_projects (prove credibility)
At least ONE exact phrase from funding_opportunity.priorities (mirror funder language)
Banned:
...
If ngo_profile.past_projects is empty → flag in warnings: "No past performance data available; executive summary may lack credibility"
If funding_opportunity.grant_amount is null → use placeholder "[AMOUNT]" and flag in warnings
ARCH_PROBLEM
...
Who Affected (1 para): Describe beneficiaries specifically (cite ngo_profile.beneficiaries + demographics)
Root Causes (1 para): Why does this persist? (systems, policies, resource gaps)
Consequences (1 para): What happens if unaddressed?
Why This NGO (1 para): Why is NGO positioned to help? (cite ngo_profile.past_projects)
Length: 500-700 words

Required Elements:

At least ONE local data point (district/county-level, NOT just national statistics)
At least ONE reference to ngo_profile.past_projects showing familiarity with this problem
If ngo_profile.beneficiary_testimonials exists, include ONE quote
Banned:
...
If no local data in ngo_profile → flag in warnings: "No local statistics available; consider adding district-level data to strengthen problem statement"
If ngo_profile.past_projects is empty → flag in warnings: "No past projects cited; cannot demonstrate NGO's familiarity with this problem"
ARCH_APPROACH
Trigger: submission_item.label contains "approach", "method", "implementation", "activities", "workplan"

Structure (MANDATORY):

Overall Approach (1 para): What is the intervention model? (cite ngo_profile.theory_of_change if exists)
...
Partnerships (1 para): Who will the NGO work with? (cite ngo_profile.partnerships)
...
At least ONE lesson learned from ngo_profile.past_projects
...
If ngo_profile.theory_of_change is null → use general approach language, flag in assumptions
If ngo_profile.partnerships is empty → flag in warnings: "No partnerships cited; consider adding local partners to strengthen approach"
ARCH_ME
...
If ngo_profile.past_projects has no baseline data → flag in assumptions: "Baseline data will be collected in Month 1"
ARCH_SUSTAIN
...
If ngo_profile.revenue_models includes social enterprise/cost recovery, cite it
Banned:
...
If ngo_profile.revenue_models is null → flag in assumptions: "Sustainability will rely on follow-on grants and partnerships"
...
Use evidence from ngo_profile where relevant
Follow all anti-AI rules (Section 0.3)
6.3 GP-P02 — Submission Item Generation Prompt (v1.0)
Purpose:
Generate content for a single submission_item from requirements_json.
...
4. **Generate Content:**
   ...
   - Use only facts from ngo_profile_json, funding_opportunity_json, requirements_json
   - Follow ALL anti-AI rules (Section 0.3)
   - Weave evidence into narrative (not footnotes)
   - Include at least ONE specific reference to ngo_profile.past_projects (if archetype requires it)

5. **Track Assumptions and Evidence:**
   - If any claim relies on missing data, add to assumptions[] (e.g., "Baseline data will be collected in Month 1")
   - List all ngo_profile fields used in evidence_used[] (e.g., "ngo_profile.past_projects", "ngo_profile.mission_statement")
...
You identify required documents from requirements_json and classify their status.
...
2. **Determine Status:**
   - If doc_name matches entry in uploaded_documents_index → status = "PROVIDED"
   - If mandatory=true and NOT in uploaded_documents_index → status = "MISSING"
   - If optional and NOT in uploaded_documents_index → status = "UNKNOWN"

3. **Identify Upload-Required Items:**
   From variant.submission_items, list items where:
   - type = "UPLOAD" OR generation_allowed = false

4. **Provide Overall Advice:**
...
requirements_json missing/invalid
