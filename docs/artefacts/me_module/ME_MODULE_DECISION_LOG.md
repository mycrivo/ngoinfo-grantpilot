...
| D-082 | 2026-08-08 | **P0 close-out — harness CI, generator retirement, content digests** | Assertion-library suite enforced in CI (`smoke` job). Pack generators retired from the tree. Layer payload digest pins are content-derived (canonical JSON), not checkout bytes. Pack carries pointer to the six L5 self-check observations and diagnostics. Content checksum and answer-key payloads unmoved; L5 deterministic arm remains uncalibrated. | P0 |
| D-083 | 2026-08-08 | **Bundle export + scorecard emitter (read-only, report-only)** | Owner-gated discovery accepted; production→`ScoreableBundle` is faithful transcription from observed shapes (no key aliasing/merge). Empty `indicator_actuals` recorded as absent. Scorecard reports per layer with starvation apart; carries commit + golden version/checksum + run identity; no threshold/pass mark/expected comparison. No baseline or ratchet. Production reads remain owner-triggered. | P0 |
| D-084 | 2026-08-16 | **Export-corpus package cancelled before build; two integrity fixes land** | Changing which corpus a Layer 5 detector searches is authoring what that detector means (D-081). Empty rendered tables are a fidelity/Checker question, not forbidden-output. Deterministic L5 arm remains uncalibrated and gates nothing. Null persisted values recorded as absent (never the text `None`); commit named `exporting_tool_commit` with originating-build limitation; scorecard states the L5 corpus actually scored (persisted section prose, not the NGO-facing document). Corpus decision deferred to pattern authoring. | P0 |
| D-085 | 2026-08-16 | **Unverified delivered document vs confirmed ledger — Phase 2 Checker** | Nothing in the system currently verifies that the delivered document matches the confirmed ledger. Recorded as a Phase 2 Checker concern. Not acted on in this package. | P2 |
...
---
DECISION (2026-08-08) — P0 close-out: harness CI, generator retirement, content digests (D-082).

A proof that executes nowhere is not a proof. From this package onward the full `tests/test_p0_assertion_library.py` suite runs in the `smoke` job of `.github/workflows/smoke-test.yml` (verbose, so the run log names every test). The pack generator scripts (`scripts/audit/_tmp_build_golden_wi1.py`, `scripts/audit/_tmp_build_golden_v11_layer4.py`) are retired from the tree: the golden pack is hand-authored ground truth; an executable that can regenerate it — and that still carried the deleted exception list — is a hazard. Provenance remains in the source documents, the reconciliation record, and the stored review artefacts. Layer payload digest pins are content-derived (parse → canonical JSON → SHA-256), not raw file bytes, so the regression proof does not depend on the developer's operating system or checkout line endings. The pack itself carries `manifest.l5_reference_self_check_observations` pointing at the six recorded IDs and per-detector diagnostics in `docs/artefacts/me_module/audits/P0_PR14_INDEPENDENT_REVIEW_2026-07-28.md`. Content checksum and L1/L2/L3/L5 answer-key payloads are unmoved; Layer 5 deterministic arm remains uncalibrated and gates nothing; no detector pattern, predicate, corpus routing, or identifier list is adjusted. CI enforcement is proved by a deliberate planted failure and revert recorded on PR #14.
---
DECISION (2026-08-08) — Bundle export + scorecard emitter (D-083).

After owner-gated discovery of report `dfd17248-9b46-48d9-8bc6-5348eab44a1c` (artefact `docs/artefacts/me_module/audits/BUNDLE_EXPORT_DISCOVERY_dfd17248_2026-08-08.json`), the production→`ScoreableBundle` mapping is faithful transcription only: facts remain a keyed object; conflicts and gap questions/rationales are carried intact under their persisted keys; section claim bindings survive; empty `indicator_actuals_json` is recorded as absent and not reconstructed; report metadata is transcribed without reconciling disagreements against the fact collection. Unobserved shapes are recorded as observations and transcribed, not coerced. The scorecard emitter runs the assertion library and reports each layer with judged outcomes apart from PASS-BY-STARVATION; it carries git commit, golden dataset version and content checksum, and run identity; it does not judge, set a threshold/pass mark, or compare to any expected or prior result. No baseline or contamination ratchet in this package. Production DB/S3 reads remain owner-triggered (`scripts/audit/bundle_export_run.py`); the builder does not produce the real-report scorecard. Constructed-input tests only.
---
DECISION (2026-08-16) — Export-corpus package cancelled before build (D-084).

The Layer 5 export-corpus package was cancelled before build. The stop condition was correct: Layer 5 is not starving on an empty corpus; it is scoring persisted section prose — a real measurement of a different artefact than the NGO-facing document. Changing which corpus a detector searches is authoring what that detector means and belongs to the owner-and-CTO pattern authoring session (D-081), not a build package. The originating justification was also wrong on its own terms: an empty rendered table is content failing to appear, a fidelity question, not a forbidden-output question. The deterministic Layer 5 arm remains uncalibrated and gates nothing. Two integrity fixes from the same independent review land: (1) null persisted values are recorded as absent, never the text `None`; (2) the recorded commit is named `exporting_tool_commit`, and both bundle and scorecard state that the originating report build is not recoverable from the persisted record. The scorecard states which corpus Layer 5 actually scored and that the deterministic arm is uncalibrated and gates nothing. No corpus routing, detector, retrieval, extraction, or four-state corpus model is built.
---
DECISION (2026-08-16) — Unverified delivered document vs confirmed ledger (D-085).

Nothing in the system currently verifies that the delivered document matches the confirmed ledger. This is a Checker responsibility, recorded as a Phase 2 concern. Not acted on in the PR #15 close-out package.
---
