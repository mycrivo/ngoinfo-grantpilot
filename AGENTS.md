# AGENTS.md — NGOInfo GrantPilot backend

This file is read by every coding agent (Cursor agents and subagents, cloud agents, Claude Code). It states how work is done in this repository. Package specs, contracts and the master task list say *what* to build; this file says *how*.
...
## 4. The build law (non-negotiable)
...
4. **Anti-bent-ruler.** Never change an assertion to match behaviour. A target moves only when the correct answer has changed, and every moved target is listed in the PR.
...
- Structure comes from the report's **requirement snapshot**, never from `funder_report_templates` (retired; rows kept, no engine reads from P3 onward).
...
- **Derived numbers are computed by code.** A model never performs arithmetic; the writer cites derived facts.
- The **evidence matrix** is the only satisfaction judgement. Gates, writer and critic read it; nothing re-judges.
