...
from app.reports.worker.job_lease import lease_claim_values
from app.reports.worker.job_timeout import (
    JOB_WALL_CLOCK_CAP_SECONDS,
    fail_job_wall_clock_exceeded,
    resolve_active_job_timeout_seconds,
)
...
JOB_TIMEOUT_SECONDS = JOB_WALL_CLOCK_CAP_SECONDS
...
def _execute_job_with_timeout(job_id: UUID, timeout_seconds: float) -> None:
    """Run pipeline in a worker thread with a stage-aligned wall-clock backstop."""
...
def poll_once(*, job_timeout_seconds: float | None = None) -> int:
    ...
    resolved_timeout = job_timeout_seconds
    try:
        ...
        if resolved_timeout is None:
            resolved_timeout = resolve_active_job_timeout_seconds(session, job_id)
    finally:
        session.close()

    assert job_id is not None
    timeout = resolved_timeout if resolved_timeout is not None else JOB_TIMEOUT_SECONDS
...
def run_forever() -> None:
    ...
    if reaped:
        logger.warning("Startup orphan reaper recovered %d stale running job(s)", reaped)
    while True:
        try:
            count = poll_once()
            if count == 0:
                ...
                if reaped:
                    logger.warning(
                        "Idle-cycle orphan reaper recovered %d stale running job(s)",
                        reaped,
                    )
