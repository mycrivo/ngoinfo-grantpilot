---
name: M&E Module Orientation
overview: "Read-only orientation report: what the M&E Donor Report Writer module is, how it maps onto the existing GrantPilot backend, where code should live, known risks/drift, and readiness for Stage A governance (no product code yet)."
todos:
  - id: stage-a-governance
    content: "Stage A (next turn): .cursor/rules/, hooks, REPO_MAP, KILL_SWITCH, DECISION_LOG — no product code"
    status: pending
  - id: stage-b-spec-lock
    content: "Stage B: 4 field contracts, FUNDER_TEMPLATE_SCHEMA, API additions, NLCF+FCDO stress test (requires T2)"
    status: pending
  - id: clarify-open-items
    content: Resolve 7 clarification questions (frontend repo, plan slug, JSONB naming, artefact location, Railway Buckets, UI colours, missing docs)
    status: pending
isProject: false
---
...
**Status:** Pre-build. Stage A not started. No product code written. This turn is analysis only.
...
Four isolation rules (from [ME_MODULE_MASTER_MEMORY.md](M_E_Module/ME_MODULE_MASTER_MEMORY.md) §10):
...
**Important:** [M_E_Module/](M_E_Module/) planning artefacts stay **reference material**. Runtime code lives under `app/reports/` per the locked isolation design. Stage A will produce [`REPO_MAP_ME_MODULE.md`](M_E_Module/ME_MODULE_PROJECT_PLAN.md) at repo root documenting this boundary.
...
[`app/reports/router.py`](app/main.py) owns prefix `/api` and includes `reports` + `report_templates` sub-routers. **Core files must not import from `app.reports`.**

### Single mounting seam (frontend — spec only; not in this repo)

Per [ME_MODULE_WIREFRAMES_BRANDED.html](M_E_Module/ME_MODULE_WIREFRAMES_BRANDED.html): routes under `/reports/*`, nav entry + 8 screens hidden behind `NEXT_PUBLIC_ME_MODULE_ENABLED` (name TBD in Stage B). Frontend lives in a **separate Next.js codebase** (deployed at grantpilot.ngoinfo.org per [FRONTEND_ARCHITECTURE_SPEC.md](docs/artefacts/FRONTEND_ARCHITECTURE_SPEC.md)); **this repo is backend-only**.
...
| Topic | Conflict |
|-------|----------|
| **JSONB column names** | [ME_MODULE_ARCHITECTURE_SPEC.md](M_E_Module/ME_MODULE_ARCHITECTURE_SPEC.md) / master memory: `report_sections`, `format_rules`, `terminology_map` — [ME_MODULE_INTERNAL_ARCHITECTURE.html](M_E_Module/ME_MODULE_INTERNAL_ARCHITECTURE.html): `report_sections_json`, `format_rules_json`, `terminology_map_json` |
| **UI colour language** | Master memory §14: teal/plum/orange — wireframes + brand: purple (agent) / blue (gate) / navy (action) |
| **Missing companion docs** | Master memory references `ME_AGENTIC_REUSE_MAP.md`, `ME_COMPLIANCE_MARKET_RESEARCH.md` — **not present** in [M_E_Module/](M_E_Module/) |
...
```33:48:app/models/usage_ledger.py
...
4. **`alembic/env.py`** — imports only 7 models ([`alembic/env.py`](alembic/env.py)); `Proposal`, `StripeEvent`, `FundingOpportunity` omitted. Migrations are hand-written (works today) but autogenerate/compare would miss tables. New M&E models must be added to env imports if parity checks rely on metadata.
...
I understand the **immediate next turn is Stage A only** — governance scaffold, **no product code**:
...
[ME_MODULE_WIREFRAMES_BRANDED.html](M_E_Module/ME_MODULE_WIREFRAMES_BRANDED.html) defines **8 screens**:
...
[NGOINFO_BRAND_GUIDELINES.md](M_E_Module/NGOINFO_BRAND_GUIDELINES.md): DM Sans, navy `#1A1F71`, purple `#6D35FF`, 8px grid — aligned with wireframes. GrantPilot workspace UI also documented in [`docs/artefacts/BRAND_AND_FRONTEND_SPEC.md`](docs/artefacts/BRAND_AND_FRONTEND_SPEC.md).
...
3. **Impact Pro plan slug** — New enum value `IMPACT_PRO` (extends [`UserPlan`](app/models/user_plan.py) CHECK + Stripe price ID), or overload `IMPACT` with added entitlements?
